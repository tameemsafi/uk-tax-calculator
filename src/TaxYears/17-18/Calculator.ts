...
export default class Calculator {
  ...
  getTaxBreakdown() {
    return {
      ...
      studentLoan: {
        plan: (this.options.studentLoanPlan === StudentLoanPlans.PLAN_1 ) ? 'PLAN_1' : (this.options.studentLoanPlan === StudentLoanPlans.PLAN_2) ? 'PLAN_2' : 'NO_PLAN',
  ...
  getTaperDeductions(): number {
    ...
    let incomeMinusPensionMinusTaperThreshold: number = incomeMinusPensionContributions - this.taxSettings.allowance.thresholds.taper;
  ...
  getTotalIncomeTax(): number {
    ...
    let totalIncomeTax: number = incomeTaxBreakdown.rate_0.tax + incomeTaxBreakdown.rate_20.tax + incomeTaxBreakdown.rate_40.tax + incomeTaxBreakdown.rate_45.tax;
  ...
  getTotalTaxForRateWithIncome(taxRate: TaxRate, totalIncome: number): TaxBreakdownItem {
    let incomeTaxRateDifference: number = ( taxRate.end === -1 ) ? totalIncome : this.getAmountRounded( taxRate.end - taxRate.start );
  ...
  getNationalInsuranceBreakdown(): NationalInsuranceBreakdown {
    ...
    let rate_0: TaxBreakdownItem = this.getTotalTaxForRateWithIncome( nationalInsuranceBands.rate_0, grossWeeklyIncome );
  ...
  getTotalWeeklyNationalInsurance(): number {
    ...
    let totalWeeklyNationalInsurance: number = nationalInsuranceBreakdown.rate_0.tax + nationalInsuranceBreakdown.rate_12.tax + nationalInsuranceBreakdown.rate_2.tax;
  ...
  getTotalYearlyNationalInsuranceWithAgeDeductions(): number {
    let totalNationalInsurance: number = this.getTotalYearlyNationalInsurance() - this.getNationalInsuranceAgeRelatedDeductions();
  ...
  getTotalStudentLoanRepayment(): number {
    ...
    let studentLoanRepaymentTotal: number = this.getIncomeAboveStudentLoanThreshold() * this.getStudentLoanRepaymentRate();
  ...
  getTotalTaxDeductions(): number {
    let totalTaxDeductions: number = this.getTotalIncomeTax() + this.getTotalStudentLoanRepayment() + this.getTotalYearlyNationalInsuranceWithAgeDeductions();
